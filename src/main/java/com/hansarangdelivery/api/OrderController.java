...
public class OrderController {
    ...
    public ResultResponseDto<OrderResponseDto> createOrder(
        ...
        OrderResponseDto responseDto = orderService.createOrder(requestDto,
            userDetails.getUser());// 주문 생성 로직 실행
        return new ResultResponseDto<>("주문 생성 성공", 200, responseDto);
    }


    @GetMapping("/{orderId}")
    @PreAuthorize("hasRole('ROLE_MANAGER')")// 특정 주문 상세 정보 조회
    ...
    @GetMapping("/me/{orderId}") // 내 주문 상세 정보 조회
    public ResultResponseDto<OrderResponseDto> readMyOrder(@PathVariable("orderId") UUID orderId,
        @AuthenticationPrincipal UserDetailsImpl userDetails) {
    ...
    public ResultResponseDto<OrderResponseDto> updateOrder(@PathVariable("orderId") UUID orderId,
    ...
    public ResultResponseDto<OrderResponseDto> deleteOrder(@PathVariable("orderId") UUID orderId,
        @AuthenticationPrincipal UserDetailsImpl userDetails) {
        try {
    ...
    @GetMapping()
    public ResultResponseDto<Page<OrderResponseDto>> searchOrder(@RequestParam UUID orderId,
        Pageable pageable) {
